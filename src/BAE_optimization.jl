...
function train_BAE_traintest!(X_train::AbstractMatrix{<:AbstractFloat}, X_test::AbstractMatrix{<:AbstractFloat}, BAE::BoostingAutoencoder; soft_clustering::Bool=false, MD::Union{Nothing, MetaData}=nothing, save_data::Bool=false, path::Union{Nothing, String}=nothing)
    ...
    @showprogress for epoch in 1:BAE.HP.epochs
        ...
        loader = Flux.Data.DataLoader(Xt_train, batchsize=BAE.HP.batchsize, shuffle=true) 
        ...
        push!(sparsity_level, 100 * count(x->x!=0, BAE.coeffs) / length(BAE.coeffs)) # Percentage of nonzero elements in the encoder weight matrix (lower values are better)
    ...
    BAE.Z = get_latentRepresentation(BAE, hcat(Xt_train, Xt_test))
